...
about $10^{31}$, a hundred million times all the storage humanity has
...
few seconds of GPU") declines to pay. The wall from part 1 is back. It just
changed currency, from *bytes of state* to *entanglement the contraction
has to squeeze through*. Physicists will recognize the shape:
for volume-law circuits, treewidth grows with depth×width, and the
exponential always collects eventually.

One more honest ledger line: the *path-finding* isn't free either. It cost
1.5 s of CPU hyper-optimization at depth 40, before any GPU work. And a
single amplitude is not a simulation: sampling bitstrings from a deep
random circuit needs many amplitudes or smarter (and costlier) contraction
...
the speed of its memory and the size of its memory. Every route past that
trades exponentially in some other direction.** Which is, of course, the
whole reason quantum hardware is worth building.
