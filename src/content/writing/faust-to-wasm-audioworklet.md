...
The [`@grame/faustwasm`](https://github.com/grame-cncm/faustwasm) package
wraps libfaust in a few megabytes of WASM that runs anywhere Node or a
browser does — no toolchain, no native builds, `npm install` and you have a
DSP compiler. On my machine each instrument compiles in **40–120 ms** and
renders **100–425× faster than realtime** on one CPU core. The entire
six-piece gallery album re-renders from source in about six hundred
milliseconds, which changes your relationship with "final" mixes.
...
faustwasm's offline processor keeps DSP state across `render()` calls, so the
renderer walks the timeline in 64-sample chunks — 1.3 ms of scheduling
resolution — applying sets and linear ramps between chunks. Each voice is its
own WASM instance (part 1 explains why a ringing modal bank must not be
retuned), summed, tail-trimmed at −72 dB, edge-faded, peak-normalized to
−1 dBFS, and written as 24-bit WAV by a forty-line encoder with no
dependencies.
...
- **Every render gets a sidecar.** Next to each WAV lands a JSON with the
  ...
  faustwasm version, and the SHA-256 of the DSP source that produced it. A
...
The browser side inverts the economics. You *could* ship libfaust and
compile in the tab like the Faust IDE does — but you shouldn't make readers
download a compiler. Instead, a build step precompiles each `.dsp` to a WASM
module plus a JSON description of its parameters, and the page instantiates
them with faustwasm's small runtime loader inside an `AudioWorklet` — audio
on its own thread, untouched by scroll jank.
...
Three complete instruments in ~220 KB — inside the 300 KB
budget I keep for lazily-loaded interactive figures on this site, with all
three worklets instantiating in **50 ms** on first click. For comparison,
a single decent piano *sample* is larger than this entire orchestra.
...
trigger, measure signal on the other side.
...
handing a recording of the bar to a GPU and asking gradient descent to find
the physics, four thousand guesses at a time.
